...
- Профиль: статус, дата истечения, оставшиеся дни, **Telegram ID** (он же пароль для входа в Lampac)
...
> **Как использовать ID для входа:** после одобрения заявки пользователь открывает «📋 Мой профиль» и видит своё `🆔 123456789`. Это число нужно вставить в поле «Пароль» при подключении к Lampac — оно совпадает с полем `id` в `users.json`.
